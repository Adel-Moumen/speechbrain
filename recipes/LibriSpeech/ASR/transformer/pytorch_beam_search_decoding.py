...
class ASR(sb.core.Brain):
    ...
    def compute_objectives(self, predictions, batch, stage):
        ...
        if stage != sb.Stage.TRAIN:
            ...
            for logs in p_ctc:
                results = decoder(logs.detach().cpu().unsqueeze(0))
                tokens= results[0][0].tokens
                predictions = [filter_ctc_output(tokens.tolist(), blank_id=0)]
...
if __name__ == "__main__":
    ...
    vocab_list = [tokenizer.id_to_piece(i) for i in range(tokenizer.vocab_size())]
    

    decoder = ctc_decoder(
        lexicon=None,
        tokens=vocab_list,
        blank_token=vocab_list[0],
        sil_token=vocab_list[0],#"▁",
        beam_size=100,
        log_add=True,
        beam_threshold=50,
        beam_size_token=100,
