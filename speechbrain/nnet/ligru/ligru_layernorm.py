...
from speechbrain.nnet.ligru.cuda_ligru_cell_layernorm import _ligru_cell_cupy
...
class LiGRU_Layer(torch.nn.Module):
    ...
    def __init__(
        ...
        self.norm_hidden = nn.LayerNorm(2 * self.hidden_size, elementwise_affine=True)
        ...
        from speechbrain.nnet.ligru.cuda_ligru_cell_layernorm import _ligru_cell_jit

        # jit the cell to gain some speed up
        self._ligru_cell_jit = torch.jit.script(_ligru_cell_jit(act=nonlinearity, norm=self.norm_hidden))
    ...
    def forward(self, x, hx: Optional[Tensor] = None):
        ...
        if FLAG:
            self.lambda_regu = self._get_lambda(h, self.u.weight)

        return h

    def _get_lambda(self, h, u):
        uz, uh = u.chunk(2, 0)
        print(h.shape)
        ht_max, _ = torch.max(h[:, :-1], axis=1) 
        sz = torch.linalg.matrix_norm(uz, ord=2)   
        sh = torch.linalg.matrix_norm(uh, ord=2)

        gz, gh = self.norm_hidden.weight.chunk(2, 0)

        gzk, idxz = torch.max(gz, axis=0)
        ghk, idxh = torch.max(gh, axis=0)
        var = ht_max @ u.T
        var = var.var(-1, keepdim=False, unbiased=False)
        
        ht_max = ht_max.max()
        varz, varh = var.chunk(2, 0)
        return (ht_max / 4) * (gzk / varz[idxz]).item() * sz + (ghk / varh[idxh]).item() * sh
    ...
    def _ligru_cell_cpu(self, w, ht, drop_mask):
        ...
        drop_mask : torch.Tensor
            dropout masks.
        """
        ht,  _, _, _,  _, _  = self._ligru_cell_jit(w, self.u.weight, ht, drop_mask)
        return ht
    ...
    def _ligru_cell(self, w, ht, drop_mask):
        ...
        if w.is_cuda == ht.is_cuda == drop_mask.is_cuda is True:
            ...
            return _ligru_cell_cupy.apply(
                self._ligru_cell_jit, w, self.u.weight, ht, drop_mask, self.norm_hidden.weight, self.norm_hidden.bias
            )
        else:
...
if __name__ == "__main__" :
    ...
    inp_tensor = torch.rand([4, 1000, 2]).to("cuda")
    FLAG = True
    net = LiGRU(input_shape=inp_tensor.shape, hidden_size=128, num_layers=10).to("cuda")
    out_tensor, _= net(inp_tensor) 

    print("lmb=", net.lambda_value())