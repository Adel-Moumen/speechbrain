...
class AnyTokensTransformerLMScorer(BaseAnyTokensScorerInterface):
    ...
    def preprocess_func(self, decoded_seq):
        ...
        length_of_hyps = [len(hyp) for hyp in enc_hyps]
        ...
        return padded_hyps, length_of_hyps
    ...
    def rescore_hyps(self, hyps):
        ...
        with torch.no_grad():
            # preprocess hypotheses
            padded_hyps, length_of_hyps = self.preprocess_func(hyps)
            ...
            # mask the scores of the padded tokens
            length_of_hyps = torch.tensor(length_of_hyps, device="cuda")
            log_probs_scores = log_probs_scores.masked_fill(
                torch.arange(log_probs_scores.size(1), device="cuda") >= length_of_hyps.unsqueeze(1),
                0,
            )

            log_probs_scores = log_probs_scores.sum(dim=-1)
...
class ScorerBuilder:
    ...
    def __init__(
        self,
        weights=dict(),
        lattice_scorers=list(),
        full_scorers=list(),
        partial_scorers=list(),
        scorer_beam_scale=1.5,
    ):
        assert len(weights) == len(full_scorers) + len(lattice_scorers) + len(
            partial_scorers
        ...
        lattice_scorers_names = [
            impl.__class__.__name__.lower().split("scorer")[0]
            for impl in lattice_scorers
        ]
        ...
        self.lattice_scorers = dict(zip(lattice_scorers_names, lattice_scorers))
    ...
    def lattice_rescoring(self, scores, hyps):
        ...
        """
        for k, impl in self.lattice_scorers.items():
            if isinstance(impl, AnyTokensRNNLMScorer) or isinstance(
                ...
                if scores.device != lm_score.device:
                    scores = scores.to(lm_score.device)
